...
//                std::cout << "First lovalization if(act. else)" << std::endl;
    ...
    void Tracking::InformOnlyTracking(const bool &flag) {
        std::cout << "INFORMED LOCAL TRACKING ONLY" << std::endl;
        mbOnlyTracking = flag;
    }

    int Tracking::getLoopCloses() {
        return mpLoopClosing->getCloses();
    }

    void Tracking::ActivateLocalizationMode() {
        std::cout << "Activating localization modes" << std::endl;
        if (mpSystem)
            mpSystem->ActivateLocalizationMode();
    }

    void Tracking::DeactivateLocalizationMode() {
        std::cout << "Deactivating localization modes" << std::endl;
        if (mpSystem)
            mpSystem->DeactivateLocalizationMode();
    }

    cv::Mat Tracking::getKeyFrames() {
        ...
        list<ORB_SLAM2::KeyFrame *>::iterator lRit = this->mlpReferences.begin();
        list<double>::iterator lT = this->mlFrameTimes.begin();
        list<bool>::iterator lbL = this->mlbLost.begin();
        for (list<cv::Mat>::iterator lit = this->mlRelativeFramePoses.begin(),
                     lend = this->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++, lbL++) {
            if (*lbL)
                continue;
            ...
            // create a vector of the pose of the camera frame
            vector<float> q = Converter::toQuaternion(Rwc);
            cv::Mat new_frame;
            float time_stamp = *lT - 1500000000;
            std::vector<float> pose = {time_stamp, twc.at<float>(0, 0), twc.at<float>(0, 1), twc.at<float>(0, 2)};
            pose.insert(pose.end(), q.begin(), q.end());

            framePoses.push_back(cv::Mat(pose));
        }

//    std::cout << "Frame poses: " << std::endl << framePoses << std::endl;
        return framePoses.clone();
    }
